...
void PauseMenu::Initialize()
{
    ...
    pPanelSprite_->SetColor({ 0.09f, 0.11f, 0.17f, 0.94f });
    ...
    selectedIndex_ = kResume;
    isOpen_ = false;
    currentAction_ = PauseMenuAction::None;
    animTimer_ = 0.0f;
...
{
    isOpen_ = true;
    selectedIndex_ = kResume; // 開いた時は一番上の「ゲームへ戻る」を選択
    currentAction_ = PauseMenuAction::None;
    animTimer_ = 0.0f;
    UpdateLayout();
}
...
{
    PauseMenuAction action = currentAction_;
    currentAction_ = PauseMenuAction::None;
    return action;
}

void PauseMenu::TriggerActionByIndex(int index)
{
    switch (index)
    {
    case kResume:
        currentAction_ = PauseMenuAction::Resume;
        break;
    case kStageSelect:
        currentAction_ = PauseMenuAction::StageSelect;
        break;
    case kTitle:
        currentAction_ = PauseMenuAction::Title;
        break;
    default:
        break;
    }
...
void PauseMenu::Update(Input* pInput)
{
    if (!isOpen_)
    {
        return;
...
void PauseMenu::UpdateLayout()
{
    ...
    static const Vector4 kNormalColors[kItemCount] = {
        { 0.18f, 0.32f, 0.46f, 0.85f }, // 1. ゲームへ戻る (青緑系)
    ...
    static const Vector4 kHighlightColors[kItemCount] = {
        { 0.25f, 0.78f, 1.00f, 1.0f },  // 1. ゲームへ戻る (発光シアン)
...
void PauseMenu::DrawOverlayUI()
{
    ...
    if (ImGui::Begin("##PauseHeaderUI", nullptr, flags))
    {
        ImGui::SetWindowFontScale(1.8f);
        ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 0.95f), "P A U S E");
        ImGui::SetWindowFontScale(1.0f);
        ImGui::End();
    }

    static const char* kItemLabels[kItemCount] = {
        "1. Resume ( Game Return )",
    ...
    for (int i = 0; i < kItemCount; ++i)
    {
        ...
        std::string winName = "##PauseBtnText" + std::to_string(i);
    ...
    if (ImGui::Begin("##PauseFooterGuideUI", nullptr, flags))
    {
        ImGui::TextColored(ImVec4(0.7f, 0.75f, 0.85f, 0.9f),
            "[ Up / Down or W / S ]: Move    [ Space / Enter / Click ]: Select    [ Esc ]: Close");
        ImGui::End();
    }
#endif // _DEBUG
}
